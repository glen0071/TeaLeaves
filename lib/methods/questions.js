Meteor.methods({
  ...
  updateQuestion: function(currentQuestion, newHeadline, newText, newThemes, newDeadline) {
    return Questions.update({_id: currentQuestion}, {$set: {headline: newHeadline, text: newText, themes: newThemes, deadline: newDeadline}});
  },
  deleteQuestion: function(currentQuestion){
    return Questions.remove({_id: currentQuestion});
  },
  updateVote: function(vote, currentQuestion) {
    ...
    if(filtered.length === 0) {
      return Questions.update({_id: currentQuestion}, {$push: {answers: {userId: currentUser, answer: vote, votedOn: new Date()}}, $inc: {answerCount: 1}})
    }else{
